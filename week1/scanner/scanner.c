...
{ 
  ...
  if (currentChar == EOF) {
    state = 41;
  } else if (charCodes[currentChar] == CHAR_TIMES) { // Phát hiện mở chú thích
    state = 37;
  } else {
    state = 41;
  }
  ...
  while (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_TIMES) {
      readChar();
      if (charCodes[currentChar] == CHAR_RPAR) { // Phát hiện đóng chú thích *)
        state = 39;
        break;
      }
    } else {
  ...
  if (currentChar == EOF) {
    error(ERR_ENDOFCOMMENT, lineNo, colNo); // Báo lỗi nếu EOF trước khi kết thúc chú thích
  }
  return getToken();
